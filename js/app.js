...
function fetchCategories() {
	...
	arr.map(item => {
		...
		<div role="status" class="max-w-sm animate-pulse">
          <div class="bg-gray-50 rounded-md dark:bg-gray-300 w-full
          mb-4 p-4 flex items-center gap-4">
           <div class="w-[50px] h-[50px] bg-gray-200 dark:bg-gray-400
            rounded-md"></div>
...
function renderCategories(categories) {
	...
	categories.map(item => {
      ...
      <li
        ...
        class='flex gap-4 p-4 shadow-sm rounded-md items-center cursor-pointer bg-slate-50 hover:shadow-md transform active:scale-95 border ${selectedCategory === item.strCategory ? 'border-blue-500' : ''
			}'>
        <img 
	...
	arr.map(item => {
		...
		<div role='status' class=' animate-pulse'>
			<div class='bg-gray-50 dark:bg-gray-200 rounded-md min-h-[300px] p-4'>
				<div class='flex items-center justify-center w-full h-[230px] bg-gray-100 rounded-md dark:bg-gray-300'>
					<i class='bx bx-image-alt text-8xl text-gray-100 text-gray-400'></i>
				</div>
				<div class='w-full mt-4 mx-auto rounded-md bg-gray-100 bg-gray-300  h-[30px]'>
				</div>
			</div>
		</div>
		`
...
function renderMeals(array) {
	...
	<img height='250' width='300px' class='rounded-md' src="${item.strMealThumb}" alt="">
	<h1>${item.strMeal}</h1>
	`
	})
}
